...
# Match similar symptoms
def correct_symptom(symptom, possibilities, cutoff=0.6):
    matches = difflib.get_close_matches(symptom, possibilities, n=1, cutoff=cutoff)
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        corrected = correct_symptom(item, all_symptoms)
        if corrected:
            input_vector[symptoms_dict[corrected]] = 1
        else:
            print(f"Warning: Symptom '{item}' not recognized or corrected.")
...
def helper(dis):
    ...
    pre = prec_df[prec_df['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.tolist()
...
def index():
    if request.method == 'POST':
        ...
        return render_template('index.html', disease=predicted_disease, desc=desc, pre=pre[0], med=med, diet=die, workout=wrkout)
